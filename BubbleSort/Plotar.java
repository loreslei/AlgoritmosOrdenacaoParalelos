...
public class Plotar {
    ...
    public static void main(String[] args) {
        String caminhoCSV = "BubbleSort/resultados.csv";
        ...
        for (Dado d : dados) {
            if (d.nome.equalsIgnoreCase("Bubble Sort Paralelo")) {
        ...
        for (int threads : threadsSet) {
            ...
            List<Dado> paralelos = dados.stream()
                    .filter(d -> d.nome.equalsIgnoreCase("Bubble Sort Paralelo") && d.threads == threads)
            ...
            for (Dado p : paralelos) {
                ...
                Optional<Dado> s = dados.stream()
                        .filter(d -> d.nome.equalsIgnoreCase("Bubble Sort Serial") && d.tamanho == p.tamanho)
                        .findFirst();
            ...
            String titulo = "Bubble Sort - Serial vs Paralelo (" + threads + " Threads)";
    ...
    // Método para salvar o gráfico como PNG
    private static void salvarGraficoPNG(String titulo, int[] tamanhos, double[] temposSerial, double[] temposParalelo, String caminhoCSV, int threads) {
        try {
            // Determina o diretório do CSV
            File arquivoCSV = new File(caminhoCSV);
            File pasta = arquivoCSV.getParentFile(); // BubbleSort/
